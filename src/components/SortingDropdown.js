...
const sortingAlgorithms = {
  Simple: {
    ...
    "Selection Sort": "Finds the minimum element in the unsorted portion of the array and swaps it with the first element."
  },
  Efficient: {
    ...
    "Heap Sort": "Builds a max heap from the array and repeatedly extracts the maximum element and swaps it with the last element."
  },
  Other: {
    "Radix Sort": "Sorts integers by their individual digits.",
    "Counting Sort": "Sorts integers in a range by counting the number of occurrences of each value.",
    "Bucket Sort": "Sorts elements by distributing them into buckets and sorting each bucket.",
    "Shell Sort": "A variation of insertion sort that compares elements that are a certain distance apart."
  }
};
        ...
        {Object.entries(sortingAlgorithms).map(([category, algorithms]) => (
          <div key={category}>
            ...
            {Object.entries(algorithms).map(([name, desc]) => (
              <DropdownItem key={name} onClick={() => handleAlgorithmSelect(name, desc)}>
                {name}
              </DropdownItem>
            ))}
          </div>
